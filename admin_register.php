...
if (isset($_POST['submit'])){
    ...
    if (empty($username)||empty($password)||empty($conpassword)){
        ...
        Redirect_to('admin_register.php');
    }elseif(CheckExisting($username)){
        $_SESSION["ErrorMessage"] = "Username already exist, try another"; 
        Redirect_to('admin_register.php');
    }else{
        // Query
        global $connecting;
        $sql = "INSERT INTO admin_entity (username,pass_word)";
        $sql .= "VALUES(:usernamE,:pass_worD)";
